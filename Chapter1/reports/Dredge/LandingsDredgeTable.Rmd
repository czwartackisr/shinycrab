...
**Table 8:** Exploratory dredge results of all relevant CPUE predicted with all relevant Creek Trawl lifestage variables with 1- and 2-yr. lags previously shown to have a significant relationhsip using single regression.  Results show a subset of models ranked by AICc.   
...
crab <- read.csv("CH1_LandingsDredgeNEW.csv", stringsAsFactors = FALSE)
...
B90Landingsdredge <- filter(crab, Year > 2003) %>%
  #dplyr::select(14, 7, 3, 46, 49, 40, 36, 38, 43, 54)
...
kable(B90LandingsdredgeTable) %>%
  kable_styling(bootstrap_options = c("striped", "scale_down"))

```

```{r Dredge Table2, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE, dpi=600}
