...
```{r, echo=FALSE, warning=FALSE, include=FALSE}
LandRegressTable = read.csv("SingleLandingsRegressLandingsCPUE.csv") %>%
...
LandRegressTable2 = read.csv("SingleLandingsRegressLandings.csv") %>%
  rename("Degrees of Freedom" = Degrees.of.Freedom,
         "Explanatory Variable" = Explanatory.Variable,
         "Dependent Variable" = Dependent.Variable,
         "p-value" = p.value,
...
**Table 3:** Significant OLS regression relatioships of Landings CPUE by all lifestages from all surveys.
...
```{r non-T38, include=FALSE, echo=FALSE, warning=FALSE, dpi=600, fig.cap="Table 4: OLS regression of all non-Creek Trawl survey total CPUEs by all lifestages from all surveys."}

Land_Table2 =  kable(LandRegressTable2, "latex", booktabs = T, align = "llcccc") %>%
  kable_styling(latex_options = "scale_down" , font_size = 8) %>%
  add_header_above(c(" " = 2, "Summary Statistics" = 4)) %>%
  pack_rows("Harbor Trawl (explanatory variable)", 1,1) %>%
  pack_rows("Creek Trawl (explanatory variable)", 2,2) %>%
